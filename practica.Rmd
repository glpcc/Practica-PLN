...
```{r lema}
cambio1 <- gsub("hipo","", enfermedades3)
cambio2 <- gsub("hiper","", cambio1)
cambio3 <- gsub("algia", "", cambio2)
cambio4 <- gsub("itis","",cambio3)
form_invariable <- gsub("oma","", cambio4)
```

```{r 2}
res <- spacy_parse(form_invariable, pos = FALSE, lemma = TRUE, entity = FALSE)
res[, c("token", "lemma")]
```
```{r}
